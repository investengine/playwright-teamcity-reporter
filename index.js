class TeamCityReporter {
  ...
  logTestResults(test) {
    const testName = this.escape(test.title)
    ...
    test.results.forEach((result) => {
      ...
      switch (result.status) {
        ...
        case 'timedOut':
          const errorMessage =
            result.status === 'failed' ? result.error.message : `Timeout of ${test.timeout}ms exceeded.`
          const errorDescription = result.errors.reduce((message, error) => `${message}\n\n${error.stack || ''}`, '')
      ...
      console.log(
        `##teamcity[testFinished name='${testName}' duration='${this.escape(result.duration)}' flowId='${
          this.flowId
        }']`,
      )
    })
  ...
  onEnd() {
    ...
    this.rootSuite.allTests().forEach((testCase) => {
      this.logTestResults(testCase)
    })
  }
}
